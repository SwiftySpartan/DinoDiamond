...
void Renderer::Render(State &state) {
  ...
  SDL_SetRenderDrawColor(sdl_renderer, 133, 193, 200, 80);
	...
	// Render Stage
	// background
	state.GetBackgrounds()[4].RenderCopy(sdl_renderer, sdl_texture_back_clouds_three);
	state.GetBackgrounds()[4].RenderCopy(sdl_renderer, sdl_texture_back_clouds_two);
	state.GetBackgrounds()[4].RenderCopy(sdl_renderer, sdl_texture_back_clouds_one);
	...
	// challenge.Render(sdl_renderer); ADD CHALLENGE
